...
void steering_engine_pwm_init(void)
{
  ...
  //初始化PB0
	...
	TIM_TimeBaseInitStructure.TIM_Prescaler =83;  //计数频率1Mhz，即1us
	TIM_TimeBaseInitStructure.TIM_Period=4999;	//那么0.5-2.5ms，对应500-2500，周期宽度5000
	TIM_TimeBaseInit(STEERING_ENGINE_TIM,&TIM_TimeBaseInitStructure);
	...
	TIM_OC2Init(STEERING_ENGINE_TIM,&TIM_OCInitStructure);	

    TIM_OC2PreloadConfig(STEERING_ENGINE_TIM,TIM_OCPreload_Enable);
	TIM_ARRPreloadConfig(STEERING_ENGINE_TIM,ENABLE);

	
	TIM_Cmd(STEERING_ENGINE_TIM,ENABLE);  //使能定时器	
}
